#!/usr/bin/env python3
"""
QK 电路 2D 频谱数据收集（完整版）- 支持所有四种运算

计算注意力交互矩阵 A = W_E @ W_Q @ W_K^T @ W_E^T 的二维 DFT 分析
保存完整的 2D 频谱数据用于热图绘制

使用方法:
    python qk_circut_full.py --operation x+y
    python qk_circut_full.py --all
"""
...
def extract_qk_spectrum_from_checkpoints(checkpoint_dir, output_file, p=97, steps=None):
    """从 checkpoint 中提取 QK 电路的完整 2D 频谱数据"""
    if steps is None:
        # 默认分析的关键步数
        steps = [100, 500, 1000, 5000, 10000, 30000, 50000, 75000, 100000]

    checkpoint_files = []
    for step in steps:
        checkpoint_path = os.path.join(checkpoint_dir, f'checkpoint_step_{step}.pt')
        if os.path.exists(checkpoint_path):
            checkpoint_files.append((step, checkpoint_path))
        else:
            print(f"  警告: Checkpoint step {step} 不存在")
...
def main():
    parser = argparse.ArgumentParser(description='计算 QK 电路的完整 2D 频谱数据')
    ...
    for op_key in operations:
        ...
        output_file = f"{OPERATIONS[op_key]['data_dir']}/qk_circut_full.csv"

        if not os.path.exists(checkpoint_dir):
            print(f"  跳过: 检查点目录不存在 - {checkpoint_dir}")
            continue

        extract_qk_spectrum_from_checkpoints(checkpoint_dir, output_file, args.p)
