...
注意：对于除法，x / y (mod p) 定义为 x * y^{-1} (mod p)
其中 y^{-1} 是 y 模 p 的乘法逆元，即 y * y^{-1} ≡ 1 (mod p)
当 y = 0 时，定义结果为 0
"""
...
class Config:
    ...
    metric_file = "/root/data1/zjj/Grokking_Formulation/data/x_div_y/metric.csv"
...
class ModuloDataset(Dataset):
    ...
    def __init__(self, p, operation="div", train=True, train_ratio=0.5, seed=0):
        self.p = p
        self.operation = operation

        # 生成所有可能的 (x, y) 组合
        # 对于除法，y 不能为 0，所以跳过 y=0 的情况
        all_pairs = []
        for x in range(p):
            for y in range(1, p):  # y 从 1 开始，避免除以 0
    ...
    def __getitem__(self, idx):
        ...
        # 计算 x / y (mod p)
        # 定义 x / y ≡ x * y^{-1} (mod p)，其中 y^{-1} 是 y 模 p 的乘法逆元
        if y == 0:
            label = 0  # 特殊情况：虽然数据集已跳过 y=0，但仍需处理
        else:
            # 计算 y 的模逆元
            y_inv = pow(y, -1, self.p)  # y * y_inv ≡ 1 (mod p)
...
def train():
    ...
    print(f"Note: Division defined as x * y^(-1) (mod p)")
    print("=" * 60)
