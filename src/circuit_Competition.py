...
def main():
    ...
    n_memo_components = 10
    n_fourier_freqs = 20

    print("=" * 60)
    print("计算电路竞争（记忆 vs 算法）")
    print("=" * 60)
    print(f"Checkpoint 目录: {checkpoint_dir}")
    print(f"输出文件: {output_file}")
    print(f"记忆子空间主成分数: {n_memo_components}")
    print(f"算法子空间低频分量数: {n_fourier_freqs}")
    print("=" * 60)
    print("\n子空间定义:")
    print("  S_memo: 初始化嵌入的主成分（记忆方向）")
    print("  S_fourier: DFT 低频分量（算法方向）")
    print("=" * 60)

    os.makedirs(os.path.dirname(output_file), exist_ok=True)

    compute_circuit_competition(checkpoint_dir, output_file, n_memo_components, n_fourier_freqs)
